...
import { getEntities } from './bon-sortie.reducer';
    ...
    <div>
      <h2 id="bon-sortie-heading">
        ...
        <Link to={`${match.url}/ligne`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          ...
          &nbsp;
          <Translate contentKey="ibamApp.bonSortie.home.createLabel">Create new Bon Sortie</Translate>
        </Link>
      </h2>
          ...
          <Table responsive>
            ...
            <tbody>
              {bonSortieList.map((bonSortie, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${bonSortie.id}`} color="link" size="sm">
                      {bonSortie.id}
                    </Button>
                  </td>
                  <td>
                    {bonSortie.dateSortie ? <TextFormat type="date" value={bonSortie.dateSortie} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                  ...
                  <td>{bonSortie.remarques}</td>
                  <td>{bonSortie.projet ? <Link to={`projet/${bonSortie.projet.id}`}>{bonSortie.projet.libelle}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${bonSortie.id}`} color="info" size="sm">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${bonSortie.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${bonSortie.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
