...
const SelectTreeTable: any = selectTableHOC(treeTableHOC(ReactTable));
...
function getNodes(data, node: any = []) {
...
export default class Example extends React.Component<any, any> {
  ...
  toggleSelection = (key: any, shift, row) => {
    /*
  ...
  render() {
    const { toggleSelection, toggleAll, isSelected, logSelection, toggleType, onExpandedChange, toggleTree } = this;
    ...
    return (
      <div style={{ padding: '10px' }}>
        ...
        <ul>
          <li>
            <strong>Toggle Tree:</strong> enables or disabled the pivotBy on the table.
          </li>
          <li>
            <strong>Select Type:</strong> changes from 'checkbox' to 'radio' and back again.
          </li>
          <li>
            <strong>Log Selection to Console:</strong> open your console to see what has been selected.
          </li>
        </ul>
        <p>
          <strong>NOTE:</strong> the selection is maintained when toggling the tree on and off but is cleared when switching between select
          types (radio, checkbox).
        </p>
        <button className="btn btn-primary mr-2" onClick={toggleTree}>
          Toggle Tree [{pivotBy && pivotBy.length ? pivotBy.join(', ') : ''}]
        </button>
        <button className="btn btn-primary mr-2" onClick={toggleType}>
          Select Type: <strong>{selectType}</strong>
        </button>
        <button className="btn btn-primary" onClick={logSelection}>
