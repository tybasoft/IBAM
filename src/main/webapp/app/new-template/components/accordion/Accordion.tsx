...
class Accordion extends Component<any, any> {
  state = {
  ...
  static AccordionItem = props => (
    <Fragment>
      <div className="accordion-item" onClick={() => props.toggle(props.index)}>
    ...
    const Nodes = Children.map(this.props.children, (child: any, index) => {
      if (Boolean(child.type === Accordion.AccordionItem)) {
        return cloneElement(child, {
          index: index,
          toggle: this.toggle,
          selected: index === this.state.selectedAccordionItem ? true : false,
          ...child.props
        });
      }
    });
