...
public class MaterielResource {
     ...
     *         body the new materiel, or with status {@code 400 (Bad Request)} if
     *         the materiel has already an ID.
     ...
     *         the updated materiel, or with status {@code 400 (Bad Request)} if the
     *         materiel is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the materiel couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the materiel, or with status {@code 404 (Not Found)}.
     */
    ...
    public boolean generateReport(@PathVariable String format) {
        ...
        reportService.setDataSource((List) materielRepository.findAll());
        return reportService.exportReport(format);
    }

    @GetMapping("/materiels/consommation/{Id}")
    public ResponseEntity<?> generateCinsommationReport(@PathVariable String Id) {
        reportService.exportConsommationReportParMateriel(Long.parseLong(Id));
