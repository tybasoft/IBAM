...
public class CentreMaintenanceResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new centreMaintenance, or with status {@code 400 (Bad Request)} if the centreMaintenance has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/centre-maintenances")
    public ResponseEntity<CentreMaintenance> createCentreMaintenance(@Valid @RequestBody CentreMaintenance centreMaintenance) throws URISyntaxException {
        log.debug("REST request to save CentreMaintenance : {}", centreMaintenance);
        if (centreMaintenance.getId() != null) {
            throw new BadRequestAlertException("A new centreMaintenance cannot already have an ID", ENTITY_NAME, "idexists");
        }
        CentreMaintenance result = centreMaintenanceRepository.save(centreMaintenance);
        return ResponseEntity.created(new URI("/api/centre-maintenances/" + result.getId()))
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated centreMaintenance,
     * or with status {@code 400 (Bad Request)} if the centreMaintenance is not valid,
     * or with status {@code 500 (Internal Server Error)} if the centreMaintenance couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/centre-maintenances")
    public ResponseEntity<CentreMaintenance> updateCentreMaintenance(@Valid @RequestBody CentreMaintenance centreMaintenance) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, centreMaintenance.getId().toString()))
            .body(result);
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of centreMaintenances in body.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the centreMaintenance, or with status {@code 404 (Not Found)}.
     */
