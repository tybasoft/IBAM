...
/**
 * REST controller for managing {@link com.tybasoft.ibam.domain.TransfertMateriel}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class TransfertMaterielResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new transfertMateriel, or with status {@code 400 (Bad Request)} if the transfertMateriel has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/transfert-materiels")
    public ResponseEntity<TransfertMateriel> createTransfertMateriel(@Valid @RequestBody TransfertMateriel transfertMateriel) throws URISyntaxException {
        log.debug("REST request to save TransfertMateriel : {}", transfertMateriel);
        if (transfertMateriel.getId() != null) {
            throw new BadRequestAlertException("A new transfertMateriel cannot already have an ID", ENTITY_NAME, "idexists");
        }
        TransfertMateriel result = transfertMaterielRepository.save(transfertMateriel);
        return ResponseEntity.created(new URI("/api/transfert-materiels/" + result.getId()))
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated transfertMateriel,
     * or with status {@code 400 (Bad Request)} if the transfertMateriel is not valid,
     * or with status {@code 500 (Internal Server Error)} if the transfertMateriel couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/transfert-materiels")
    public ResponseEntity<TransfertMateriel> updateTransfertMateriel(@Valid @RequestBody TransfertMateriel transfertMateriel) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, transfertMateriel.getId().toString()))
            .body(result);
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of transfertMateriels in body.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the transfertMateriel, or with status {@code 404 (Not Found)}.
     */
